...
{
	struct pcie_msi_bridge_ctrl *ctrl = irq_data_get_irq_chip_data(d);
	phys_addr_t addr = ctrl->doorbell_phys + HWINQ_TO_REG_OFF(d->hwirq);
...
static void sophgo_pcie_msi_parent_domain_free(struct pcie_msi_bridge_ctrl *ctrl,
					       unsigned int hwirq,
					       unsigned int nr_irqs)
{
	...
	bitmap_release_region(ctrl->irq_bitmap, hwirq - ctrl->fwspec.param[0], order_base_2(nr_irqs));
	spin_unlock_irqrestore(&ctrl->lock, flags);
}

static int sophgo_pcie_msi_parent_domain_alloc(struct irq_domain *domain,
					       unsigned int virq,
					       unsigned int hwirq,
					       unsigned int nr_irqs)
{
	...
	fwspec.fwnode = ctrl->fwspec.fwnode;
	fwspec.param_count = 2;
	fwspec.param[0] = ctrl->fwspec.param[0] + hwirq;
	fwspec.param[1] = ctrl->fwspec.param[1];

	ret = irq_domain_alloc_irqs_parent(domain, virq, nr_irqs, &fwspec);
	if (ret < 0)
		sophgo_pcie_msi_parent_domain_free(ctrl, virq, nr_irqs);
					...
					void *args)
{
	...
	ret = sophgo_pcie_msi_parent_domain_alloc(domain, virq, ret, nr_irqs);
	if (ret < 0) {
					...
					unsigned int virq, unsigned int nr_irqs)
{
	...
	sophgo_pcie_msi_parent_domain_free(ctrl, data->hwirq, nr_irqs);
}
...
static int sophgo_pcie_msi_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u64(dev->of_node, "sophgo,doorbell-phys", &addr);
	if (ret)
		return ret;

	ctrl->doorbell_phys = addr;
