...
{
	if (en) {
		WARN(sg2044_pll_poll_update(pll) < 0, "fail to lock pll\n");
...
static int sg2044_pll_update_vcosel(struct sg2044_pll *pll, u64 rate)
{
	...
	pr_info("pll: %s: set rate vcosel 0x%08lx\n",
		clk_hw_get_name(&pll->common.hw),
		FIELD_PREP(PLL_VCOSEL_MASK, sel));

	return regmap_update_bits(pll->top_syscon,
			       ...
			       unsigned long rate, unsigned long parent_rate)
{
	...
	pr_info("pll: %s: set rate value 0x%08x\n", clk_hw_get_name(hw), value);
	...
	pr_info("pll: %s: set rate disable OK\n", clk_hw_get_name(hw));

	sg2044_pll_update_vcosel(pll, vco);

	regmap_update_bits(pll->top_syscon,
			   pll->syscon_offset + pll->pll.ctrl_offset +
	...
	{
		u32 ctrl1, ctrl2, enable, status;

		regmap_read(pll->top_syscon,
			    pll->syscon_offset + pll->pll.ctrl_offset,
			    &ctrl1);
		regmap_read(pll->top_syscon,
			    pll->syscon_offset + pll->pll.ctrl_offset + PLL_HIGH_CTRL_OFFSET,
		...
		pr_info("pll: %s: set status 0x%08x 0x%08x 0x%08x 0x%08x\n",
			clk_hw_get_name(hw), ctrl1, ctrl2, enable, status);
	}
				   ...
				   u32 value)
{
	...
	pr_info("div: %s: set rate with reg 0x%08x\n",
			       ...
			       unsigned long rate, unsigned long parent_rate)
{
	...
	pr_info("div: %s: set rate %lu with div %u\n",
		clk_hw_get_name(&div->common.hw),
		divider_recalc_rate(&div->common.hw, parent_rate, value,
				    NULL, div->div.flags, div->div.width),
		value);
				  ...
				  void *data)
{
	...
	pr_info("mux: %s: enter notifier\n", clk_hw_get_name(hw));

	if (event == PRE_RATE_CHANGE) {
		mux->saved_parent = ops->get_parent(hw);
		if (mux->saved_parent)
			ret = ops->set_parent(hw, 0);
		pr_info("mux: %s: switch from %u to %u\n",
			clk_hw_get_name(hw), mux->saved_parent, 0);
	} else if (event == POST_RATE_CHANGE) {
		ret = ops->set_parent(hw, mux->saved_parent);
		pr_info("mux: %s: switch from %u to %u\n",
			clk_hw_get_name(hw), 0, mux->saved_parent);
	}
				...
				const struct sg2044_clk_desc_data *desc)
{
	...
	for (i = 0; i < desc->num_mux; i++) {
		...
		if (!(mux->mux.flags & CLK_MUX_READ_ONLY)) {
			...
			dev_info(dev, "register notifier for %s\n", clk_hw_get_name(hw));
		}
